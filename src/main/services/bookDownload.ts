...
async function runDownload(params: {
  ...
  try {
    const nodeReadable = Readable.fromWeb(response.body as unknown as NodeWebReadableStream);
    nodeReadable.on('data', (chunk: Buffer) => {
      bytesReceived += chunk.length;
      if (bytesReceived > entry.sizeBytes) {
        nodeReadable.destroy(new Error('Downloaded more bytes than the declared size.'));
        return;
      }
      onProgress?.({ contentId: entry.contentId, bytesReceived, totalBytes: entry.sizeBytes, phase: 'downloading' });
    });
    await pipeline(nodeReadable, writeStream, { signal });
  } catch (err) {
