...
export async function runMsixFirmwarePlumbingProbe(): Promise<MsixFirmwarePlumbingProbeResult> {
  ...
  // Harmless stand-in for a vendor tool: prints a marker and exits 0. No network, no hardware,
  // no vendor binary of any kind.
  fs.writeFileSync(probeToolPath, ['@echo off', `echo ${EXPECTED_MARKER}`, 'exit /b 0', ''].join('\r\n'), 'utf-8');
